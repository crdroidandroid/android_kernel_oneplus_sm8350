...
uint32_t get_64align_addr(uint32_t data_base) {

	uint32_t multiple = 0;
	uint32_t surplus = 0;
	uint32_t addr_64align = 0;
...
static uint32_t eeprom_64align_write(struct cam_eeprom_ctrl_t *e_ctrl,
	struct cam_write_eeprom_t *cam_write_eeprom) {
	...
	m_eeprom_size = cam_write_eeprom->calibDataSize;
	align_addr = get_64align_addr(cam_write_eeprom->baseAddr);
	part1_length = align_addr - cam_write_eeprom->baseAddr;
	if(part1_length > m_eeprom_size) {
		part1_length = m_eeprom_size;
	}
	part2_length = m_eeprom_size - part1_length;

	CAM_INFO(CAM_EEPROM, "part1_length: %d, part2_length: %d", part1_length, part2_length);
	if (part1_length) {
		for (i = 0; i < part1_length;) {
			i2c_reg_settings.size = 0;
			star_addr = (cam_write_eeprom->baseAddr + i);
			for (j = 0; j < WRITE_EEPROM_MAX_LENGTH && i < part1_length; j++) {
				i2c_reg_arrays[j].reg_addr = star_addr;
				i2c_reg_arrays[j].reg_data = cam_write_eeprom->calibData[i];
				i2c_reg_arrays[j].delay = 0;
				i2c_reg_settings.size++;
	...
	if (part2_length) {
		for (i = 0; i < part2_length;) {
			i2c_reg_settings.size = 0;
			star_addr = (align_addr + i);
			for (j = 0; j < WRITE_EEPROM_MAX_LENGTH && i < part2_length; j++) {
				i2c_reg_arrays[j].reg_addr = star_addr;
				i2c_reg_arrays[j].reg_data = cam_write_eeprom->calibData[i + part1_length];
				i2c_reg_arrays[j].delay = 0;
				i2c_reg_settings.size++;
				i++;
			}
	...
	struct cam_write_eeprom_t *cam_write_eeprom)
{
	...
	if (cam_write_eeprom->isWRP == 0x01) {
		...
		rc = camera_io_dev_read(&e_ctrl->io_master_info,
			 ...
			 CAM_ERR(CAM_EEPROM, "cam: readcalibData: 0x%x WRPaddr 0x%x.", readcalibData, cam_write_eeprom->WRPaddr);
		if (rc) {
			CAM_ERR(CAM_EEPROM, "read WRPaddr failed rc %d.  WRPaddr 0x%x OpenWRP 0x%x CloseWRP 0x%x",
				rc,
				cam_write_eeprom->WRPaddr,
				cam_write_eeprom->OpenWRP,
				cam_write_eeprom->CloseWRP);
			return rc;
		}
		if (readcalibData != cam_write_eeprom->CloseWRP) {
	...
	if (((cam_write_eeprom->cam_id == 0x01) || (cam_write_eeprom->cam_id==0x02) || (cam_write_eeprom->cam_id==0x03))) {
		...
		if (get_64align_addr(cam_write_eeprom->baseAddr) != cam_write_eeprom->baseAddr) {
			rc = eeprom_64align_write(e_ctrl, cam_write_eeprom);
			if (rc) {
				CAM_ERR(CAM_EEPROM, "eeprom_64align_write failed rc %d", rc);
				return rc;
			}
		} else {
			for (i = 0; i < m_eeprom_size;) {
	...
	if (cam_write_eeprom->isWRP == 0x01) {
		...
		if(readcalibData != cam_write_eeprom->OpenWRP){
			rc = camera_io_dev_write(&e_ctrl->io_master_info, &i2c_reg_settings);
			if (rc) {
				CAM_ERR(CAM_EEPROM, "write WRPaddr failed rc %d",rc);
				return rc;
