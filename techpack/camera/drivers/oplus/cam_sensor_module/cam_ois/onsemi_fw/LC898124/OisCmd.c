...
TRACE("1st PtoP =%08x \n", ((UINT16)0xFFFF - (StTneVal.StDwdVal.UsHigVal + StTneVal.StDwdVal.UsLowVal)) ) ;	
			// \8F\89\89\F1\82\CCPtp\82Ŋ\F9\82\C9Target Range\82ɓ\FC\82\C1\82Ă\A2\82\E9\82Ȃ\E7\8FI\97\B9\81B
...
void	SetSinWavePara( UINT8 UcTableVal ,  UINT8 UcMethodVal )
{
	...
	if( UcMethodVal == 255/*CIRCWAVE*/) {
		...
		RamWrite32A( SinWave_Phase	,	0x60000000 ) ;		// \90\B3\8C\B7\94g\82̈ʑ\8A\97\CA
		RamWrite32A( CosWave_Phase 	,	0x60000000 );		// \90\B3\8C\B7\94g\82̈ʑ\8A\97\CA
	}
	...
	if( UlFreqDat == 0xFFFFFFFF )			/* Sine\94g\92\86\8E~ */
	{
		...
		SetTransDataAdr124( SinWave_OutAddr	,	0x00000000 ) ;		// \8Fo\97͐\E6\83A\83h\83\8C\83X
		SetTransDataAdr124( CosWave_OutAddr	,	0x00000000 );		// \8Fo\97͐\E6\83A\83h\83\8C\83X
		...
		RamWrite32A( SinWaveC_Regsiter	,	0x00000001 ) ;									// Sine Wave Start
		SetTransDataAdr124( SinWave_OutAddr	,	(UINT32)HALL_RAM_HXOFF1 ) ;		// \8Fo\97͐\E6\83A\83h\83\8C\83X
		SetTransDataAdr124( CosWave_OutAddr	,	(UINT32)HALL_RAM_HYOFF1 ) ;		// \8Fo\97͐\E6\83A\83h\83\8C\83X
...
//TRACE( "MIN AFTER Y\t=\t0x%04X\r\n", y_min_after ) ;
	...
	// \83}\81[\83W\83\93\82\AA\82܂\C1\82\BD\82\AD\82Ȃ\A2\82\E0\82͕̂s\97ǂƂ\B7\82\E9
	if (x_max_after < gout_x) {
		...
		// \83}\81[\83W\83\93\83I\81[\83o\81[\82ł\A0\82\EA\82΁AADOFFSET\82\F0\8DX\90V\82\B7\82\E9
...
//+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=
//+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=
//+=  \82\B1\82\B1\82\A9\82\E7\92ǉ\C1 by K.Otake                                                                     +=
//+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=
//+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=
//********************************************************************************
...
//    mixval->hy45x = -(sin(mixval->radianX) / cos(mixval->radianX - mixval->radianY));
	...
	mixval->hxsx = (unsigned char)abs(mixval->hx45x);                                     // >1\82Ȃ\E7\82΃V\83t\83g\90\94\82Ƃ\B5\82Đݒ\E8
	mixval->hysx = (unsigned char)abs(mixval->hy45y);                                     // >1\82Ȃ\E7\82΃V\83t\83g\90\94\82Ƃ\B5\82Đݒ\E8

    mixval->hx45x = mixval->hx45x / pow(2, (double)mixval->hxsx);        // \83V\83t\83g\82\F0\89\C1\96\A1\82\B5\82čČv\8EZ
    mixval->hy45y = mixval->hy45y / pow(2, (double)mixval->hysx);        // \83V\83t\83g\82\F0\89\C1\96\A1\82\B5\82čČv\8EZ
