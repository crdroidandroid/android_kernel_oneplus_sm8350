...
//--------------------------------------------------------------------------------
	...
	// Program RAM\82\CCCheckSum\82̋N\93\AE
	data[0] = 0xF0;											//CmdID
	data[1] = 0x0E;											//CmdID
	data[2] = (unsigned char)((ptr->SizeUpdataCode >> 8) & 0x000000FF);	//\8F\91\82\AB\8D\9E\82݃f\81[\83^(MSB)
	data[3] = (unsigned char)(ptr->SizeUpdataCode & 0x000000FF);			//\8F\91\82\AB\8D\9E\82݃f\81[\83^
	data[4] = 0x00;											//\8F\91\82\AB\8D\9E\82݃f\81[\83^
	data[5] = 0x00;											//\8F\91\82\AB\8D\9E\82݃f\81[\83^(LSB)

	CntWrt( data, 6 ) ;

	// CheckSum\82̏I\97\B9\94\BB\92\E8
	UlCnt = 0;
	do{
		WitTim( 1 );
		if( UlCnt++ > 10 ) {
		...
		RamRead32A( 0x0088, &UlReadVal );					// PmCheck.ExecFlag\82̓ǂݏo\82\B5
	}while ( UlReadVal != 0 );

#if 0
	// CheckSum\92l\82̓ǂݏo\82\B5
	data[0] = 0xF0;											// CmdID
	data[1] = 0x0E;											// CmdID
	CntWrt( data, 2 ) ;
	CntRd( 0, ReadData , 8 );
	...
	IOWrite32A( FLASHROM_FLAMODE , 0x00000002);
	// CheckSum\92l\82̔\BB\92\E8(\8A\FA\91Ғl\82́AHeader\82\C9define\82\B3\82\EA\82Ă\A2\82\E9)
	for( i=0; i<8; i++) {
		if(ReadData[7-i] != *p++ ) {  							// CheckSum Code\82̔\BB\92\E8
...
UINT_8 EraseUserMat128(UINT_8 StartBlock, UINT_8 EndBlock )
{
	...
	//***** User Mat\82̃u\83\8D\83b\83N\8F\C1\8B\8E *****
	for( i=StartBlock ; i<EndBlock ; i++) {
		RamWrite32A( 0xF00A, ( i << 10 ) );				// FromCmd.Addr\82̐ݒ\E8
		RamWrite32A( 0xF00C, 0x00000020 );				// FromCmd.Control\82̐ݒ\E8(\83u\83\8D\83b\83N\8F\C1\8B\8E)

		// \83u\83\8D\83b\83N\8F\C1\8B\8E\82̏I\97\B9\94\BB\92\E8
		WitTim( 5 );
		UlCnt = 0;
		do{
//			WitTim( 4 );
			...
			RamRead32A( 0xF00C, &UlReadVal );					// FromCmd.Control\82̓ǂݏo\82\B5
		}while ( UlReadVal != 0 );
	}
	IOWrite32A( 0xE0701C , 0x00000002);
	return(0);
...
UINT_8 ProgramFlash128_LongBurst( DOWNLOAD_TBL_EXT* ptr )
{
	...
	RamWrite32A( 0xF007, 0x00000000 );						// FlashAccess Setup
//	RamWrite32A( 0xF00A, 0x00000000 );						// FromCmd.Addr\82̐ݒ\E8
	RamWrite32A( 0xF00A, 0x00000030 );						// FromCmd.Addr\82̐ݒ\E8
	...
	{
		if( ++UcOddEvn >1 )  	UcOddEvn = 0;	// \8A\8B\F4\90\94Check
		...
		UlCnt = 0;
		if(UcOddEvn == 0){
			do{															// \8F\91\82\AB\8D\9E\82݂̏I\97\B9\94\BB\92\E8
				RamRead32A( 0xF00C, &UlReadVal );						// FromCmd.Control\82̓ǂݏo\82\B5
				if( UlCnt++ > 100 ) {
					IOWrite32A( 0xE0701C , 0x00000002);
...
UINT_8 ProgramFlash128_Standard( DOWNLOAD_TBL_EXT* ptr )
{
	...
	IOWrite32A( 0xE0701C , 0x00000000);
	RamWrite32A( 0xF007, 0x00000000 );						// FlashAccess Setup
	RamWrite32A( 0xF00A, 0x00000010 );						// FromCmd.Addr\82̐ݒ\E8
	data[0] = 0xF0;						// CmdH
	data[1] = 0x08;						// CmdL
	data[2] = 0x00;						// FromCmd.BufferA\82̃A\83h\83\8C\83X
...
TRACE("[%d]UcOddEvn= %d , data[1]= %d \n", i, data[1], NcFromVal );
		...
		// 32Byte\82Ȃ\E7\82΁A2\89\F1\82ɕ\AA\82\AF\82đ\97\82\E7\82Ȃ\A2\82Ƃ\A2\82\AF\82Ȃ\A2\81B
		data[2] = 0x00;
		UlNum = 3;
		for(j=0 ; j < BURST_LENGTH_FC; j++){
			data[UlNum++] = *NcFromVal++;
		...
		RamWrite32A( 0xF00B, 0x00000010 );							// FromCmd.Length\82̐ݒ\E8
		UlCnt = 0;
		if (UcOddEvn == 0){
			do{															// \8F\91\82\AB\8D\9E\82݂̏I\97\B9\94\BB\92\E8
				RamRead32A( 0xF00C, &UlReadVal );						// FromCmd.Control\82̓ǂݏo\82\B5
		 	...
		 	RamWrite32A( 0xF00C, 0x00000004 );	// FromCmd.Control\82̐ݒ\E8(\8F\91\82\AB\8D\9E\82\DD)
		}else{
			do{															// \8F\91\82\AB\8D\9E\82݂̏I\97\B9\94\BB\92\E8
				RamRead32A( 0xF00C, &UlReadVal );						// FromCmd.Control\82̓ǂݏo\82\B5
				if( UlCnt++ > 250 ) {
			...
			RamWrite32A( 0xF00C, 0x00000008 );	// FromCmd.Control\82̐ݒ\E8(\8F\91\82\AB\8D\9E\82\DD)
		}			
	}
...
//Infomat\82̊m\94F\81B\82\E0\82\B5Re-Adjust\82\B5\82Ă\A2\82Ȃ\A2\82Ȃ\E7\8DĎ\C0\8Ds\81B
	ans = CheckDrvOffAdj();
	if( ans == 1 ){
...
//--------------------------------------------------------------------------------
	...
	IOWrite32A( 0xE0701C , 0x00000000);
	RamWrite32A( 0xF00A, 0x00000000 );									// FromCmd.Addr\82̐ݒ\E8
	RamWrite32A( 0xF00D, ptr->SizeFromCodeValid );						// \97L\8C\F8CheckSum\83T\83C\83Y\82̐ݒ\E8

	RamWrite32A( 0xF00C, 0x00000100 );									// FromCmd.Control\82̐ݒ\E8(CheckSum\8E\C0\8Ds)
	WitTim( 6 );
	UlCnt = 0;
	do{																	// \8F\91\82\AB\8D\9E\82݂̏I\97\B9\94\BB\92\E8
		RamRead32A( 0xF00C, &UlReadVal );								// FromCmd.Control\82̓ǂݏo\82\B5
	...
	RamRead32A( 0xF00D, &UlReadVal );									// CheckSum\92l\82̓ǂݏo\82\B5
...
UINT_8	FlashBlockErase( UINT_8 SelMat , UINT_32 SetAddress )
{
	...
	if( SelMat != USER_MAT && SelMat != INF_MAT0 && SelMat != INF_MAT1 && SelMat != INF_MAT2  )	return 10;	// INF_MAT2\82\E0Access\82\B5\82Ȃ\A2
	// reject command if address inner NVR3
	if( SetAddress > 0x000003FF )											return 9;

	// Flash write\8F\80\94\F5
	ans	= UnlockCodeSet();
...
UINT_8	FlashMultiRead( UINT_8 SelMat, UINT_32 UlAddress, UINT_32 *PulData , UINT_8 UcLength )
{
	...
	// fail safe
	// reject irregular mat
	if( SelMat != USER_MAT && SelMat != INF_MAT0 && SelMat != INF_MAT1 && SelMat != INF_MAT2  )	return 10;	// INF_MAT2\82\CDRead only Access\82\B5\82Ȃ\A2
	// reject command if address inner NVR3
	if( UlAddress > 0x000003FF )											return 9;
...
UINT_8	FlashBlockWrite( UINT_8 SelMat , UINT_32 SetAddress , UINT_32 *PulData)
{
	...
	// fail safe
	// reject irregular mat
//	if( SelMat != INF_MAT0 && SelMat != INF_MAT1  )			return 10;	// USR MAT,INF_MAT2\82\E0Access\82\B5\82Ȃ\A2
	if( SelMat != INF_MAT0 && SelMat != INF_MAT1 && SelMat != INF_MAT2  )			return 10;	// USR MAT\82\CDAccess\82\B5\82Ȃ\A2
	// 
	if( SetAddress > 0x000003FF )							return 9;
	...
	IOWrite32A( FLASHROM_FLA_ADR, ((UINT_32)SelMat << 16) | ( SetAddress & 0x000010 )) ;// address\82\CDpage\82̂ݎw\92\E8
	// page write Start
	IOWrite32A( FLASHROM_FLAMODE , 0x00000000);
...
TRACE( "WrHallCalData : Mode = %d\n", UcMode);
	/* Back up ******************************************************/
	ans =FlashMultiRead( INF_MAT0, 0, UlMAT0, 32 );	// check sum \88ȊO
	if( ans )	return( 1 );
...
TRACE( "[ %d ] = %08x\n", i, UlMAT0[i] );
	...
	//*****************************************************//
		/* X \82\C6 Y \82\C5Step \8Bɐ\AB\82\AA\88Ⴄ\8E\9E */
...
/*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*/
/* function name    : lstsq								 							  	 */
/* input parameter  : double x[] X\8E\B2\82̔z\97\F1\83f\81[\83^                                         */
/*                    double y[] Y\8E\B2\82̔z\97\F1\83f\81[\83^                                         */
/*                    int n      \83f\81[\83^\8C\94                                              */
/*                    int m      \8Bߎ\97\91\BD\8D\80\8E\AE\8E\9F\90\94                                          */
/* output parameter : double c[] \8Bߎ\97\82\B5\82\BD\8Ae\8D\80\82̌W\90\94\94z\97\F1                                  */
/*                    \96߂\E8\92l\82́A0\82Ő\B3\8F\ED\81A-1\82Ŏ\FB\91\A9\8E\B8\94s                                    */
/* comment          : \8Dŏ\AC2\8F\E6\8Bߎ\97\96@                                                      */
/*				   	   	    			      								  2018.03.07 */
/*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*/
...
//	RamWrite32A( 0xF007, 0x00000000 );						// FlashAccess Setup
	...
	do{
		...
		RamRead32A( 0x5005, &UlReadVal );					// complite Flag\82̓ǂݏo\82\B5
	}while ( UlReadVal != 0 );
	...
	do{
		...
		RamRead32A( 0x5006, &UlReadVal );					// complite Flag\82̓ǂݏo\82\B5
	}while ( UlReadVal != 0 );
	IOWrite32A( 0xE0701C , 0x00000002);
